...
def create_pie_chart(data, column, title):
    """
    Cria um gráfico de pizza a partir dos dados com compatibilidade de tema escuro.

    Args:
        data (pd.DataFrame): DataFrame com os dados.
        column (str): Nome da coluna a ser usada para as fatias.
        title (str): Título do gráfico.

    Returns:
        plotly.graph_objs.Figure: Gráfico de pizza com o tema aplicado.
    """
    ...
    fig = px.pie(
        ...
        names=column,
        title=title,
        hole=0.4,  # opcional: cria um gráfico de donut para melhor visual
    )

    # Ajustes manuais de layout antes de aplicar o tema
    fig.update_traces(
        textposition='inside',
    ...
    # Aplicar o tema personalizado por último
    fig = apply_theme_to_plotly_chart(fig)

    # Forçar aplicação explícita de cores de fundo e fontes
    fig.update_layout(
        paper_bgcolor=theme_colors.get('paper_bgcolor', '#000000'),
        plot_bgcolor=theme_colors.get('plot_bgcolor', '#000000'),
        font=dict(color=theme_colors.get('font_color', '#FFFFFF')),
        legend=dict(font=dict(color=theme_colors.get('font_color', '#FFFFFF')))
    )
...
def create_bar_chart(data, x, y, title):
    ...
    # Aplicar tema diretamente sem sobrescrever configurações depois
...
def show_dashboard():
    ...
    with tab1:
        ...
        with col1:
            ...
            # Forçar configuração explícita de cores para tema escuro
            theme_colors = get_theme_colors()
            fig.update_layout(
        ...
        with col2:
            ...
            if categorias_valores:
                ...
                # Preparar dados para a função
                labels = df_categorias['Categoria'].tolist()
                values = df_categorias['Valor'].tolist()

                # Criar gráfico com tema integrado
                fig_pizza = create_pie_chart(labels, values, title='Distribuição de Despesas por Categoria')
